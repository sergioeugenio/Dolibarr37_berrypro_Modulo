 ...
 */
class pdf_expedition_berrypro_a4val extends ModelePdfExpedition
{
	...
	function __construct($db=0)
	{
		...
		$this->db = $db;
		$this->name = "BerryPro-A4-Val";
		$this->description = "BerryPro A4 Valorado";
		...
		if ($conf->expedition->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $num; $i++)
				{
					...
					$pdf->MultiCell(($this->posxup - $this->posxqtytoship), 3, $object->lines[$i]->qty_shipped,'','C');
					
					// Unit price before discount
					//Tenemos que coger los valores de importe del objeto presupuesto
					$up_excl_tax = pdf_getlineupexcltax($object, $i, $outputlangs, $hidedetails, $hookmanager);
					$pdf->SetXY($this->posxup, $curY);
					$pdf->MultiCell(($this->posxtotalht - $this->posxup - 2), 3, $up_excl_tax, '','R');
					
					// Total HT line
					$total_excl_tax = pdf_getlinetotalexcltax($object, $i, $outputlangs, $hidedetails, $hookmanager);
					$pdf->SetXY($this->posxtotalht, $curY);
					$pdf->MultiCell(($this->page_largeur - $this->marge_droite - $this->posxtotalht), 3, $total_excl_tax, 0,'R', 0);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			$pdf->line($this->marge_gauche, $tab_top+5, $this->page_largeur-$this->marge_droite, $tab_top+5); // Linea horizantal borde inferior fila encabezamiento tabla

			$pdf->SetXY($this->posxdesc-1, $tab_top+1);
			$pdf->MultiCell($this->posxqtyordered - $this->posxdesc, 2, $outputlangs->transnoentities("Description"), '', 'L');
		}

		$pdf->line($this->posxqtyordered-1, $tab_top, $this->posxqtyordered-1, $tab_top + $tab_height); //Línea vertical borde izqdo columna cantidad pedida
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxqtyordered, $tab_top+1);
			$pdf->MultiCell(($this->posxqtytoship - $this->posxqtyordered), 2, $outputlangs->transnoentities("Pedido"),'','C');
		}

		$pdf->line($this->posxqtytoship-1, $tab_top, $this->posxqtytoship-1, $tab_top + $tab_height);//Línea vertical borde izqdo columna cantidad enviada
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxqtytoship, $tab_top+1);
			$pdf->MultiCell(($this->posxup - $this->posxqtytoship), 2, $outputlangs->transnoentities("Enviado"),'','C');
		}
		//Añadidas para poner columna de precio unitario e importe
		$pdf->line($this->posxup-1, $tab_top, $this->posxup-1, $tab_top + $tab_height); //Línea vertical borde izqdo columna precio unitario
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxup, $tab_top+1);
			$pdf->MultiCell(($this->posxup - $this->posxqtytoship), 2, $outputlangs->transnoentities("Precio"),'','C');
		}
		$pdf->line($this->posxtotalht-1, $tab_top, $this->posxtotalht-1, $tab_top + $tab_height); //Línea vertical borde izqdo columna importe
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxtotalht, $tab_top+1);
			$pdf->MultiCell(($this->page_largeur - $this->marge_droite - $this->posxtotalht), 2, $outputlangs->transnoentities("Importe"),'','C');
		}
	}
