...
/**
 *	\file       htdocs/core/modules/facture/doc/pdf_berrypro.modules.php
 *	\filebase       htdocs/core/modules/facture/doc/pdf_crabe.modules.php
 *	\ingroup    facture
 *	\brief      Copiado de la plantilla original. Incluye diversas modificaciones para personalizarla.
 *				-Redefinición de espacios.
 *				-Comprobación del número de cuenta bancaria seleccionado en cada factura.
 */
 ...
 */
class pdf_berrypro extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
...
//			$pdf->MultiCell(100, 3, "Aceptamos pago mediante Pagaré a la orden.", '', 'L');
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(128,2, $outputlangs->transnoentities("Designation"),'','L');
		}

		if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
		{
			$pdf->line($this->posxtva+3, $tab_top, $this->posxtva+3, $tab_top + $tab_height);
			if (empty($hidetop))
			{
				$pdf->SetXY($this->posxtva-3, $tab_top+1);
				$pdf->MultiCell($this->posxup-$this->posxtva+7, 2, $outputlangs->transnoentities("VAT"),'','C');
			}
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $hookmanager)
	{
		...
		//Añadido para que muestre el CIF del cliente debajo del Código de Cliente
		if ($object->client->idprof1)
		{
			$posy+=3;
			$pdf->SetXY($posx,$posy);
			$pdf->SetTextColor(0,0,60);
			$pdf->MultiCell(100, 3, $outputlangs->transnoentities("ProfId1ES")." : " . $outputlangs->transnoentities($object->client->idprof1), '', 'R');
		}
		//
		$posy+=1;
