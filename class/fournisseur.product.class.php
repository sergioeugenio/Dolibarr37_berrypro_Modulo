 ...
 */
class ProductFournisseurNew extends Product
{
    var $db;
    var $error;
    ...
    /**   Muestra sólo precio del producto
     *
     *  @param  int     $showunitprice  Show "Unit price" into output string
     *  @param  int     $showsuptitle   Show "Supplier" into output string
     *  @return string                  String with supplier price
     */
    function display_only_price_product_fournisseur($showunitprice=1,$showsuptitle=1)
    {
        global $langs;
        $langs->load("suppliers");
        $out=($showunitprice?price($this->fourn_unitprice).' '.$langs->trans("HT").' &nbsp; (':'').($showsuptitle?$langs->trans("Supplier").': ':'').$this->getSocNomUrl(1);
        return $out;
    }

    function display_only_ref_supplier($showunitprice=1,$showsuptitle=1)
    {
