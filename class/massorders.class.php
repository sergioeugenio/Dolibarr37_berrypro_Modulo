...
class Massorders extends CommonObject
{
			...
			if (! $error)
			{
				...
				if ($id>0)
				{
						...
						if ($result > 0)
						{
							...
							if($conf->global->MASSO_SHOW_ORIGIN){
								$result = $object->addline(
										//$id,
										$langs->trans("Order").' '.$srcobject->ref,
										0,
										0,
										0,
										0,
					...
					if($validateInvoice){
						...
						if ($object->mode_reglement_id==3 && ! empty($conf->prelevement->enabled) && $conf->global->MASSO_WITHDRAWAL)
						{
					...
					while ($ii < $nn)
					{
						...
						if ($result > 0)
						{
							...
							if($conf->global->MASSO_SHOW_ORIGIN){
								$result = $object->addline(
										//$id,
										$langs->trans("Proposal").' '.$srcobject->ref,
										0,
										0,
										0,
										0,
	...
	function invoicing_shipment($socid, $closeOrders, $validateInvoice, $date='', $paymode=-1)
	{
		require_once DOL_DOCUMENT_ROOT.'/compta/facture/class/facture.class.php';
		require_once DOL_DOCUMENT_ROOT.'/societe/class/societe.class.php';
					...
					while ($ii < $nn)
					{
						...
						if ($result > 0)
						{
							...
							if($conf->global->MASSO_SHOW_ORIGIN){
								$result = $object->addline(
										//$id,
										$langs->trans("Sending").' '.$srcobject->ref,
										0,
										0,
										0,
										0,
	...
	function send_mail($fac,$mode){
		...
		if($conf->webmail->enabled){
			...
			$_POST['receiver'] = $array['id']?$array['id']:'thirdparty';
			...
			$reshook=$hookmanager->executeHooks('doActions','',$fac,$action);    // Note that $action and $object may have been modified by some hooks
