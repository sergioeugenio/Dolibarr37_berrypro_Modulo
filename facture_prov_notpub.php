...
/**
 *       \file       berrypro/facture_prov_notpub.php
 *       \filebase       htdocs/fourn/facture/list.php
...
$sql.= " fac.total_ht, fac.total_ttc, fac.paye as paye, fac.fk_statut as fk_statut, fac.note_public";//cambiar etiqueta por nota pública en la consulta
...
if (GETPOST("search_note_public"))//cambiar la busqueda de etiqueta por nota publica
{
    $sql .= natural_search('fac.note_public', GETPOST('search_note_public'));//cambiar etiqueta por nota pública
}
...
if ($resql)
{
	...
	if (GETPOST("search_note_public"))      $param.='&search_note_public='.urlencode(GETPOST("search_note_public"));//cambiar busqueda de etiqueta por nota pública
	...
	print_liste_field_titre($langs->trans("PublicNote"),$_SERVER["PHP_SELF"],"fac.note_public","",$param,"",$sortfield,$sortorder);//cambiar campo etiqueta por nota pública
	...
	print '<td class="liste_titre" align="left">';
	print '<input class="flat" size="16" type="text" name="search_note_public" value="'.GETPOST("search_note_public").'">';//cambiar input de buqueda de etiqueta por nota pública
	print '</td>';
	print '<td class="liste_titre" align="left">';
	...
	while ($i < min($num,$limit))
	{
		...
		print '</td>';
		print '<td>'.dol_trunc($obj->note_public,70).'</td>';//modificado etiqueta a nota publica y la cantidad de caracteres mostrados de 36 a 70
		print '<td>';
