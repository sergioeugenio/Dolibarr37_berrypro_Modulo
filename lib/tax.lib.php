...
function vat_by_date($db, $y, $q, $date_start, $date_end, $modetax, $direction, $m=0)
{
    ...
    if ($modetax == 1)  // Option vat on delivery for goods (payment) and debit invoice for services
    {
        ...
        if (! empty($conf->global->MAIN_MODULE_COMPTABILITE))
        {
            ...
            $sql.= " f.".$invoicefieldref." as facnum, f.fk_soc, f.type, f.total_ttc as ftotal_ttc,";//Añadido f.fk_soc para sacar el cliente
            $sql.= " s.nom as nom,";//Añadido para sacar el cliente
            $sql.= " p.rowid as pid, p.ref as pref, p.fk_product_type as ptype,";
            $sql.= " 0 as payment_id, 0 as payment_amount";
            $sql.= " FROM ".MAIN_DB_PREFIX.$invoicetable." as f,";
            $sql.= " ".MAIN_DB_PREFIX."societe as s," ;//Añadido para sacar el cliente
            $sql.= " ".MAIN_DB_PREFIX.$invoicedettable." as d" ;
            $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."product as p on d.fk_product = p.rowid";
            $sql.= " WHERE f.entity = " . $conf->entity;
            $sql.= " AND f.fk_statut in (1,2)"; // Validated or paid (partially or completely)
            $sql.= " AND f.fk_soc = s.rowid"; //Añadido para sacar el cliente
            if (! empty($conf->global->FACTURE_DEPOSITS_ARE_JUST_PAYMENTS)) $sql.= " AND f.type IN (0,1,2)";
            else $sql.= " AND f.type IN (0,1,2,3)";
    ...
    if ($sql != 'TODO')
    {
        ...
        if ($resql)
        {
            ...
            while($assoc = $db->fetch_array($resql))
            {
                ...
                $list[$assoc['rate']]['nom'][] = $assoc['nom']; //Añadido para sacar el cliente
