...
/**
 *	\file       	berrypro/list_prod_notpub.php
 *	\filebase       	htdocs/comm/propal/list.php
 *	\ingroup    	propal
 *	\brief      	Listado de Presupuestos con Nota Pública
 */
...
$search_publicnote=GETPOST('search_publicnote','alpha');//modificado a nota pública
...
if (GETPOST("button_removefilter_x"))
{
    ...
    $search_sale='';
    $search_ref='';
    $search_publicnote='';//modificado a nota pública
    $search_societe='';
    $search_montant_ht='';
    $search_author='';
    $search_town='';
...
$sql.= ' p.rowid as propalid, p.note_private, p.note_public, p.total_ht, p.ref, p.ref_client, p.fk_statut, p.fk_user_author, p.datep as dp, p.fin_validite as dfv,';//añadido a la consulta la nota pública
...
if ($search_publicnote) {
	$sql .= natural_search('p.note_public', $search_publicnote);//modificacion de busqueda en nota pública
}
if ($search_societe) {
...
if ($result)
{
    ...
    if ($search_publicnote) $param.='&search_publicnote=' .$search_publicnote;//modificación a nota pública
	...
	print_liste_field_titre($langs->trans('PublicNote'),$_SERVER["PHP_SELF"],'p.note_public','',$param,'',$sortfield,$sortorder);//modificado ref. cliente por nota pública
	...
	print '</td>';
	print '<td class="liste_titre">';
	print '<input class="flat" size="6" type="text" name="search_publicnote" value="'.$search_publicnote.'">';
	print '</td>';
	...
	while ($i < min($num,$limit))
	{
		...
		print $objp->note_public;//modificación nota pública
		print '</td>';
