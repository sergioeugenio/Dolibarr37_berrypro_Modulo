...
/**
 *	\file       htdocs/custom/berrypro/list_commande_products.php
...
$sql.= ' cd.fk_product, cd.description, cd.qty';
...
$sql.= ' AND c.fk_statut IN (1,2,3)';	// If module expedition disabled, we include order with status 'sending in process' into 'validated'
...
if ($resql)
{
	...
	print '<tr class="liste_titre">';
	print_liste_field_titre($langs->trans('Product'),$_SERVER["PHP_SELF"],'cd.fk_product','',$param,'width="5%"',$sortfield,$sortorder);
  print_liste_field_titre($langs->trans('Description'),$_SERVER["PHP_SELF"],'cd.description','',$param,'width="25%"',$sortfield,$sortorder);
  print_liste_field_titre($langs->trans('Stock'),$_SERVER["PHP_SELF"],'cd.qty','',$param,'width="5%"',$sortfield,$sortorder);
