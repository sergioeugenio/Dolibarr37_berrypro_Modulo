...
$product_fourn = new ProductFournisseurNew($db);
...
*/
	...
	if ($user->rights->produit->lire || $user->rights->service->lire)
	{
		...
		if (count($prods_arbo) > 0)
		{
			...
			print '<td width="2%">Ref.BerryPro</td>';
			print '<td width="30%">Descripcion</td>';
			print '<td>Cantidad</td>';
			print '<td width="60px">Stock</td>';
			print '<td>Proveedor</td>';
			print '<td>Ref. Prov.</td>';
			...
			foreach($prods_arbo as $value)
			{
				...
				if ($value['level'] <= 1)
				{
					...
					//Columna 2 es la Descripción
					print '<td>'.$productstatic->getNomUrl(1,'composition').'</td>';

					// Columna 3 Cantidad de elementos necesarios
					print '<td align="center">'.$value['nb'].'</td>';
					// Columna 4 Stock existente
					if (! empty($conf->stock->enabled)) print '<td align="center">'.$value['stock'].'</td>';	// Real stock
					// Columna 5 Proveedor con el Precio mínimo de compra
					print '<td align="left">';
					if ($product_fourn->find_min_price_product_fournisseur($productstatic->id) > 0)
					{
						//print $langs->trans("BuyingPriceMinShort").': ';
				    	if ($product_fourn->product_fourn_price_id > 0) print $product_fourn->display_only_price_product_fournisseur(0,0);
				    	else { print $langs->trans("NotDefined"); $notdefined++; $atleastonenotdefined++; }
   					// Columna 6 Referencia Proveedor
				    	print '<td>'.$product_fourn->display_only_ref_supplier(0,0).'</td>';
					}
					print '</td>';
					$unitprice=price2num($product_fourn->fourn_unitprice, 'MT');
					$totalline=price2num($value['nb'] * $product_fourn->fourn_unitprice, 'MT');
					$total+=$totalline;
					// Columna 7 Precio Mínimo de Compra
					print '<td align="center">'.($notdefined?'':price($unitprice,'','',0,0,-1,$conf->currency)).'</td>';
