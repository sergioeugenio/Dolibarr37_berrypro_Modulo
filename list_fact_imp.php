...
/**
 *		\file       /berrypro/list_fact_imp.php
 *		\filebase       htdocs/fourn/facture/impayees.php
 *		\ingroup    facture
...
//variables de la fecha actual y array con los nombre de los meses
$meses= array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
$mes_actual=date("n");
$anio_actual=date("Y");
isset($_POST['anterior']) ? $meses_anteriores=$_POST['anterior'] : $meses_anteriores=4;
isset($_POST['posterior']) ? $meses_posteriores=$_POST['posterior'] : $meses_posteriores=2;
$mes_inicial=$mes_actual-$meses_anteriores;
$meses_totales=$meses_anteriores+$meses_posteriores;
$posterior=1;

if ($user->rights->fournisseur->facture->lire)
{
?>
		Mes actual: <?php print $meses[$mes_actual-1]; ?><br />

		<form method='post'>
			<label for="anterior">Número de meses anteriores:</label>
			<select name="anterior">
			<?php
				for ($k=1;$k<=10;$k++)
				{
					$meses_anteriores==$k ? print "<option selected>".$k."</option>" : print "<option>".$k."</option>";
				}
			?>
			</select>
			<label for="posterior">número de meses posteriores:</label>
			<select name="posterior">
				<?php
				for ($k=1;$k<=10;$k++)
				{
					$meses_posteriores==$k ? print "<option selected>".$k."</option>" : print "<option>".$k."</option>";
				}
			?>
			</select>

			<input type="submit" name="" value="Mostrar meses">
		</form>
<?php
}


for ($j=$meses_totales;$j>=0;$j--)//crea un bucle para que muestre las tablas acorde con cada mes
{
	$mes=$mes_inicial;
	$anio=$anio_actual;
	$mes+=$j;
	if ($mes == $mes_actual) $posterior=0;
	if ($mes>12) $mes-=12;
	if ($mes<1) $mes+=12;
	if ($mes<$mes_inicial) $anio+=1;
	if ($posterior == 0 && $mes>$mes_actual) $anio-=1;
	...
	if ($user->rights->fournisseur->facture->lire)
	{
		...
		$sql.= " AND MONTH(f.date_lim_reglement) LIKE ".$mes; //añade a la consulta original el mes
		$sql.= " AND YEAR(f.date_lim_reglement) LIKE ".$anio; //añade a la consulta original el año
		...
		if ($resql)
		{
			...
			//print_fiche_titre($titre,$link); // se comenta para que no imprima el titulo de la pagina emcima de cada tabla
			...
			print "<td colspan='9'>".$meses[$mes-1]." ".$anio."</td>";
			print "</tr>\n";
